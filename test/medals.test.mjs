...
// SVG shield, per the mapping the user gave directly: green "10 KG" plate
// = bronze, blue "20 KG" = silver, gray "5 KG" = gold. Non-tiered medals
...
test("tiered (pr) medals render the mapped weight-plate image, not the SVG shield", async () => {
  ...
  assert.ok(bronze.querySelector("img.medal-shape")?.src.includes("assets/medal-bronze.png"));
  assert.ok(silver.querySelector("img.medal-shape")?.src.includes("assets/medal-silver.png"));
  assert.ok(gold.querySelector("img.medal-shape")?.src.includes("assets/medal-gold.png"));
...
test("tiered (streak) medals use the same tier->plate mapping as pr medals", async () => {
  ...
  assert.ok(streakBronze.querySelector("img.medal-shape")?.src.includes("assets/medal-bronze.png"));
});
...
test("non-tiered medals (milestone, rx, capstone) still render the SVG shield/circle, unaffected by the plate swap", async () => {
  ...
  for (const badge of [milestone, capstone]) {
    ...
    assert.equal(badge.querySelector("img.medal-shape"), null, "non-tiered medals should never pick up a plate image (no tier)");
  }
});
...
test("a tiered medal still gets the locked/earned CSS classes on the surrounding badge, same as before the plate swap", async () => {
  ...
  window.openAchievements();
  const bronze = badgeByName(window, "ברונזה");
  assert.ok(bronze.classList.contains("locked"), "a fresh install should have this tier locked");
  assert.ok(!bronze.classList.contains("earned"));
  // The image itself always carries medal-shape, so the existing
  // grayscale/glow CSS filters (scoped to .medal-badge.locked/.earned
  // .medal-shape) still apply to it exactly as they did to the SVG.
  assert.ok(bronze.querySelector("img.medal-shape"));
});
